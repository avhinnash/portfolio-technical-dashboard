...
crypto_values_now = {coin: CRYPTO_QTY[coin] * float(prices[ticker]) for coin, ticker in CRYPTO_TICKERS.items()}
crypto_market_value = float(sum(crypto_values_now.values()))
default_total = STOCKS_OPTIONS_DEFAULT + crypto_market_value

c1, c2, c3 = st.columns(3)
with c1:
    total_now = st.number_input(
        "Current total portfolio value ($)", min_value=1.0,
        value=float(st.session_state.get("trans_total", default_total)),
        step=5000.0, format="%.0f", key="allocation_total",
    )
    st.caption(f"Default = $53,000 of stocks/options plus the live market value of BTC, ETH, SOL, AVAX and LINK (${crypto_market_value:,.0f} currently). MSTR is included inside the $53,000 stocks/options amount, so it is not double-counted.")
with c2:
    mstr_shares = st.number_input(
        "Fixed MSTR shares", min_value=0,
        value=int(st.session_state.get("trans_mstr", 200)), step=100,
        key="allocation_mstr_shares",
    )
    st.caption("MSTR is treated as part of the Crypto slice for risk/allocation reporting, but its market value is part of the $53,000 stocks/options default.")
with c3:
...
with t1:
    ...
    st.caption("New money is added only to the diversified core, which dilutes crypto exposure without selling it.")
with t2:
...
with t3:
    ...
    st.caption("Applied to BTC, ETH, SOL, AVAX, and LINK as a group.")
with t4:
    ...
    st.caption("Expected annual return on the MSTR sleeve after the covered-call strategy.")

mstr_now = float(mstr_shares) * float(prices["MSTR"])
crypto_plus_mstr_now = crypto_market_value + mstr_now
...
st.caption("The headline Crypto slice is BTC + ETH + SOL + AVAX + LINK + MSTR. The future chart assumes no sales of these positions; their market values change through the return assumptions, while all new contributions are directed to the core portfolio in your existing target proportions.")